...
public boolean writeBbs(bBsDto dto) { // bbswriteAf.jsp에서 실행되는 메소드로 dto에는 new bBsDto(id, title, content)의 데이터값으로 넣는다.
    ...
    public boolean answer(int seq, bBsDto bbs) {  // seq = 부모의 시퀀스 bbs = 새로 입력한 답글의 데이터
     ...
     public List<bBsDto> getBbsSearchList(String choice, String search) { 
    	 // bBsDto의 데이터를 List값으로 리턴하는 메소드, bbslist.jsp에서 실행
		...
		if(choice.equals("title")) { //option에서 고른 title
			sWord = " WHERE TITLE LIKE '%" + search + "%' ";
		}else if(choice.equals("content")) { //option에서 고른 content
			sWord = " WHERE CONTENT LIKE '%" + search + "%' ";
		}else if(choice.equals("id")) { //option에서 고른 id
