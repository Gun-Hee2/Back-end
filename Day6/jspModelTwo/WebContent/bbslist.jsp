...
// 검색창에 검색어를 입력하지 않거나 항목을 고르지 않았을 때 null값을 가진다.
   choice = "";
   search = "";
}
...
$(document).ready(function() {
   // 검색어가 있는 경우
   let search = "<%=search %>";  /* 검색창에 썼던 검색어를 검색창에 그대로 둔다. */
   if(search == "") return; /* 검색창에 검색어가 없으면 종료 */
   
   // select 처리
   let obj = document.getElementById("choice"); /* 객체 obj에 검색목록으로 고른 항목으로 넣는다. */
   obj.value = "<%=choice %>"; /* 항목에 값을 고른 항목으로 넣는다. */
   obj.setAttribute("selected", "selected");   /* <select>객체에서 선택된 항목으로 설정 */
});
</script>
...
<br>   
   <a href="bbs?param=bbswrite">글쓰기</a> <!-- 글쓰기 링크를 누르면 글을 작성하는 페이지인 bbswrite.jsp로 이동한다. -->
</div>
